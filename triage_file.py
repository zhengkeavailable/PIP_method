...
def build_decision_tree_model(model, x, Trt,Trt_is,y,propensity_model,treatment_mean,a_start, b_start, z_start,constraint_value,D, N, B_lb, M_ub, epsilon,epsilon_1,epsilon_2, p,num_j,iterations,base_rate,enlargement_rate,shrinkage_rate,pip_max_rate,estimator):
    model=model.copy()
    a = {}
    a_abs = {}
    b = {}
    ...
    for t in range(2 ** D):
        ...
        for s in range(N):
            ...
            if constraint_value[t][s] > epsilon_1:
                ...
                J1_count+=1
                for k in A_R[t]:
                    model.addConstr(gp.quicksum(a[k][i] * x[s][i] for i in range(p)) - b[k] >= B_lb * (1 - z_start[t][s]))
                for k in A_L[t]:
                    model.addConstr(-(gp.quicksum(a[k][i] * x[s][i] for i in range(p))) + b[k] - epsilon >=  B_lb * (1 - z_start[t][s]))
                ...
                for k in A_L[t]:
                    model.addConstr(-(gp.quicksum(a[k][i] * x[s][i] for i in range(p))) + b[k] - epsilon >= B_lb * (1 - z[s][t]))
            else:
                J2_set[t].append(s)
                model.remove(z[s][t])
                J2_count+=1
    ...
    for t in range(2 ** D):
        for j in range(num_j):
            if estimator=='IPW':
                model.addConstr(L[t] <= gp.quicksum((y[s]*Trt_is[s][j]*z[s][t]/cps.propensity_score(propensity_model, x[s], Trt[s]-1)) for s in J0_set[t]) +
                                gp.quicksum((y[s]*Trt_is[s][j]*z_start[t][s]/cps.propensity_score(propensity_model, x[s], Trt[s]-1)) for s in J1_set[t]) + M_ub*(1 - c[t][j])) 
            elif estimator=='DR':
                model.addConstr(L[t] <= gp.quicksum(((y[s]-treatment_mean[s][Trt[s]-1])*Trt_is[s][j]*z[s][t]/propensity_model[s][Trt[s]-1]+treatment_mean[s][j] *z[s][t]) for s in J0_set[t]) +
                                gp.quicksum(((y[s]-treatment_mean[s][Trt[s]-1])*Trt_is[s][j]*z_start[t][s]/propensity_model[s][Trt[s]-1]+treatment_mean[s][j] *z_start[t][s]) for s in J1_set[t]) + M_ub*(1 - c[t][j])) 
    # ||a_k||_1 <= u_k
    for k in range(2 ** D - 1):
        for i in range(p):
    ...
    for t in range(2 ** D):
        for j in range(num_j):
           model.addConstr(M[t][j] >= gp.quicksum(z[s][t] for s in J0_set[t]) + sum(z_start[t][s] for s in J1_set[t]) - M_ub * (1 - c[t][j]))
    ...
    for t in range(2**D):
        ...
        for j in range(num_j):
            if c[t][j].X >= 0.99:
                if estimator=='IPW':
                    optimal_L[t] = sum((y[s]*Trt_is[s][j]*z[s][t].X/cps.propensity_score(propensity_model, x[s], Trt[s]-1)) for s in J0_set[t]) + sum((y[s]*Trt_is[s][j]*z_start[t][s]/cps.propensity_score(propensity_model, x[s], Trt[s]-1)) for s in J1_set[t]) + M_ub*(1 - c[t][j].X)  
                elif estimator=='DR':
                    optimal_L[t] = sum(((y[s]-treatment_mean[s][Trt[s]-1])*Trt_is[s][j]*z[s][t].X/propensity_model[s][Trt[s]-1]+treatment_mean[s][j]*z[s][t].X) for s in J0_set[t]) + sum(((y[s]-treatment_mean[s][Trt[s]-1])*Trt_is[s][j]*z_start[t][s]/propensity_model[s][Trt[s]-1]+treatment_mean[s][j]*z_start[t][s]) for s in J1_set[t]) + M_ub*(1 - c[t][j].X)  
    ...
    with open('output/output_iter='+str(iterations)+'.txt', 'a') as f:
        ...
        for s in range(N):
            ...
            for t in range(2**D):
                ...
                if constraint_value[t][s] > np.percentile(value_positive, min(enlargement_rate*base_rate,pip_max_rate)):
                    print("If enlargement,"+str(s)+','+str(t)+' in J1_set',file=f)
                elif constraint_value[t][s] < np.percentile(value_negative, 100-min(enlargement_rate*base_rate,pip_max_rate)):
            ...
            while k < 2**D - 1:
                if sum(a[k][i].X * x[s][i] for i in range(p)) - b[k].X >= 0:
                    ...
                    no_branch+=1
                    print('Large epsilon!',sum(a[k][i].X * x[s][i] for i in range(p)) - b[k].X,-(sum(a[k][i].X * x[s][i] for i in range(p))) + b[k].X - epsilon,file=f)
                    k = 2**D - 1
